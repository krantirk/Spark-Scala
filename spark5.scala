val df1 = spark.read.format("csv").option("header","True").option("inferSchema","true").option("delimiter","|").load("s3a://karna-spark-bucket/inputdatasets/TelecomData/AP_VTM.TXT_3")
val df1 = spark.read.format("csv").options(Map(("header","True"),("inferSchema","true"),("delimiter","|"))).load("s3a://karna-spark-bucket/inputdatasets/TelecomData/AP_VTM.TXT_3")
df1.persist
df1.count
df1.columns
df1.columns.size
